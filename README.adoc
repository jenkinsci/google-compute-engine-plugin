...
The plugin's link:./testimages/linux/[`testimages/linux/`] directory contains a working Packer setup that builds a Debian 12 image with Temurin 21 JRE:
...
The plugin's link:./testimages/windows/[`testimages/windows/`] directory contains a working Packer setup that builds a Windows Server 2022 image with Temurin 21 JRE, OpenSSH, and a `jenkins` admin user (password-based):
...
[cols=4*]
|===
| *Common keys* (all modes): `device-name`, `mode` (`ro`
| `rw`, default `rw`), `interface` (`SCSI`
| `NVME`), `auto-delete` (`yes`
| `no`, default `yes`).
|===
...
| `serviceAccountEmail`
| GCP service account attached to the VM, controlling what GCP APIs it can access from the metadata server. Scoped to `+https://www.googleapis.com/auth/cloud-platform+`.
|===
...
. *IAM roles*: the service account needs `compute.instanceAdmin`, `compute.networkAdmin`, and `iam.serviceAccountUser`.
