...
When a GCE zone has no available capacity (`ZONE_RESOURCE_POOL_EXHAUSTED`), the plugin can automatically provision in another zone instead.

Set *Fallback Zones* (`fallbackZones`) to a comma- or space-separated list of zones to try, in order, when the primary zone is exhausted. Zones are recommended to be in the same region as the primary zone, since a configuration's subnetwork is region-scoped. A zone in a different region may or may not provision depending on the subnetwork configuration; to reliably provision in a different region, create a separate instance configuration instead.
...
The primary zone is always tried first. When a zone returns a capacity error, it is skipped for a short cooldown (2 minutes by default) and the next provisioning cycle tries the next zone in the list. Once every zone is in cooldown, the configuration provisions nothing for that cycle, letting another configuration or cloud serve the label. Other errors (for example, quota, IAM, or an invalid configuration) do not trigger fallback.
