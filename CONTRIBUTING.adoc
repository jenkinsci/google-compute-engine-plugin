...
The default username is `jenkins` and the default password is `jenkins`. To use a custom password, set the `JENKINS_PASSWORD` environment variable before running the script:
...
export GOOGLE_JENKINS_PASSWORD=jenkins  # default; must match the password used during image build
----
