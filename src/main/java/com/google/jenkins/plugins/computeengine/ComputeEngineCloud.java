...
public class ComputeEngineCloud extends AbstractCloudImpl {
  ...
  protected Object readResolve() {
    try {
      ClientFactory clientFactory =
          new ClientFactory(
              Jenkins.getInstance(), new ArrayList<DomainRequirement>(), credentialsId);
                        ...
                        while ((System.currentTimeMillis() - startTime)
                          ...
                          try {
                            ...
                            LOGGER.log(
                                Level.WARNING,
                                String.format(
                                    "Exception waiting for node %s to connect", node.getNodeName()),
                                e);
                          }
                          return node;
                        }
                        LOGGER.log(
                            Level.WARNING, "Failed to connect to node within launch timeout");
                        return node;
                      }
                    }),
  ...
  private synchronized ComputeEngineInstance getAnAgent(
      InstanceConfiguration config, Label requiredLabel) {
    try {
